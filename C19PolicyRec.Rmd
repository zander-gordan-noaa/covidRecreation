...
library(vtable)
st(tripsTMPS %>% filter(Mode == 7) %>% select("Trips","Hrsf","Party","Stringency"),
...
texreg(list(pm),custom.model.names = c("Private"),single.row = FALSE,digits=3,label="regResults",
       caption = "Quasi-Poisson Fixed Effect Regression of Trips on Covid-19 Stringency",
       ...
       custom.gof.rows = list("Scale" = c(summary(pm)$dispersion)))
...
```{r sumSatOtherModes,echo=FALSE,warning=FALSE,results='asis',message=FALSE}

st(tripsTMPS %>% 
     mutate(Mode = recode(Mode, `3` = "Shore", `5` = "Charter", `7` = "Private")) %>% 
     select("Trips","Hrsf","Party","Stringency", "Mode") %>% 
     filter(Mode %in% c("Shore", "Charter")),
   group = 'Mode', group.test = FALSE,group.long = TRUE,
   title="Summary Statistics for Charter and Shore Modes",
   out="latex")

```


```{r regTblOtherModes,echo=FALSE,warning=FALSE,results='asis'}

staten=gsub("State","",names(coef(pm)))[2:16]

texreg(list(cm,sm),custom.model.names = c("Charter","Shore"),single.row = FALSE,digits=3,label="regResults",
       caption = "Quasi-Poisson Fixed Effect Regression of Trips on Covid-19 Stringency for Charter and Shore Modes",
       #custom.coef.names = c("Intercept",staten,month.name[-1],
       #                       "Stringency/100","(Stringency/100)^2"),
       custom.coef.map=list("(Intercept)" = "Intercept",
                            "I(Stringency/100)"="Stringency/100","I((Stringency/100)^2)"="(Stringency/100)^2"),
       include.aic=FALSE,include.bic=FALSE,include.loglik=FALSE,caption.above = TRUE,
       custom.note = "%stars. The base state is Alabama and the base Month is January.",
       custom.gof.rows = list("Scale" = c(summary(cm)$dispersion,summary(sm)$dispersion)))
