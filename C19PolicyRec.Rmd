...
the winter months in colder states.^[The [MRIP Data User Handbook](https://media.fisheries.noaa.gov/2021-09/MRIP%20Data%20User%20Handbook%20Updated%202021-09-30.pdf) explains this in Table 1: many states are not sampled during January and February, and Maine for instance is not sampled in March or April either. In the [MRIP Survey Design Manual](https://media.fisheries.noaa.gov/2021-09/MRIP-Survey-Design-and-Statistical-Methods-2021-09-15.pdf), Figure 6 shows the state-month's which do not have coverage in white, along with detail of the 2020 data quality for state-months which are covered. Since these observations are non-randomly missing, there is no reason to estimate a hurdle or 2-stage model, and we simply drop these state-months from our estimation samples.] The only states for which data is 
...
The MRIP data collection process was partially impacted by COVID conditions, however,
the main data that we rely on was able to continue unhindered. Specifically, the 
FES mail survey was able to continue, which is what is necessary for the estimation
of effort (trips) in each state-month. The APAIS however was impacted by COVID in that
many access points were closed and the survey was unable to be conducted during March
and April of 2020. If it were not for this fact, it would be possible to investigate
other outcomes such as the mean hours fished on a trip, the average distance traveled
to the fishing site, or other attributes of trips which come from the APAIS data.
...
It is important to note that these results for the impact of COVID stringency on
fishing trips represent a composite effect of factors correlated with COVID stringency.
For instance, to the degree that traffic lessened in proportion to stringency, the
travel cost to visit fishing sites was reduced and therefore our results are partially
driven by that change.
...
In future research, it may be possible to assess the impact of COVID on fishing activity
in different ways. For instance, one data source that could be utilized is foot
traffic data based on cellphone GPS tracking. In this way it would be possible to 
measure the level of activity near the coastline on a much finer spatial and temporal scale
than is possible with the survey data we rely on in this paper. It would be a challenge
to distinguish between activities such as beach going and other uses of the coastline 
versus fishing-specific activity with this data source, and additionally it would
likely not be possible to assess boat-based activity, however the advantages
in terms of data resolution may still make it an attractive option.
...
In order to assess the robustness of our results, this appendix considers in 
further detail the data limitations of the MRIP dataset used to estimate our
dependent variable of resident marine sportfishing trips per month. The MRIP
program operates with a limited budget, and for this reason the number of completed
surveys for state-months with low expected fishing activity can be quite low, which
can create substantial uncertainty in the estimated number of fishing trips taken
by residents in the state-month. We first characterize the extent of this uncertainty,
and then repeat our main regression modeling with restricted estimation samples
that exclude state-months with the most uncertainty in the estimated number of trips.

```{r PSEtable}
# load data with PSE's
...
# edit "0" PSE's for tabulation
trips_joined_for_table <- trips_joined %>% 
  mutate(pse = case_when(pse == 0 ~ 100,
                         TRUE ~ pse))
# create datasets that will be used for estimation
snds<- tripsTMPS %>% 
  filter(Mode == "7") %>% 
  group_by(State, Month) %>% 
  ...
  filter(min_Trips == 0) %>% 
  select(State, Month)
d4nma <- trips_joined_for_table %>% 
  anti_join(snds, by = c("State", "Month")) %>% 
  filter(Mode == 7)
snds5<- tripsTMPS %>%
  filter(Mode == "5") %>%
  group_by(State, Month) %>%
  ...
  filter(min_Trips == 0) %>%
  select(State, Month)
d4nma5 <- trips_joined_for_table %>%
  anti_join(snds5, by = c("State", "Month")) %>% 
  filter(Mode == 5)
snds3<- tripsTMPS %>%
  filter(Mode == "3") %>%
  group_by(State, Month) %>%
  ...
  filter(min_Trips == 0) %>%
  select(State, Month)
d4nma3 <- trips_joined_for_table %>%
  anti_join(snds3, by = c("State", "Month")) %>%
  filter(Mode == 3)
# create table with PSE quantiles by dataset
list(d4nma3, d4nma5, d4nma) %>% 
  ...
  kableExtra::kbl(booktabs = T,
                  caption = "PSE Quantiles by mode")
```

The dataset used in the main paper does not represent a full, balanced panel of
the 16 states with a monthly time step from January 2017 through December 2021. This
is because, due to the reasons explained above, some state-months (such as January
in Maine) are removed from the dataset for all years because there is no data for
these months. The key question is to what extent it is therefore appropriate to
treat the estimated number of trips as being 0. In the main text of the paper, if
a state-month has no data in every year, the state-month is removed from the dataset.
This results in a total of 35 state-months being removed for the Shore and Private
mode datasets, and 45 state-months being removed for the Charter dataset. Subtracting
these amounts from the total 192 state-months and multiplying by 5 years gives the
total number of observations for our estimation samples in the main paper, 785 and
735 respectively for Private/Shore and Charter.

However, if a state-month has data in some but not all years, it is kept in the dataset
and for the years without data a value of 0 trips is assigned as the estimated
number of trips. This happens for instance with Connecticut in the month of March
for its Charter mode data, which has complete intercept survey data in 2018, but
not in any other year. Thus, in the dataset used in the main paper, Connecticut has
an estimated 68 trips for March 2018 in the charter mode dataset, and 0 for March
of all other years. 

In this appendix, we define an alternative dataset with a more
restrictive criterion for inclusion of these problematic state-months with limited
data. In the restricted estimation sample, if a state-month is missing data for
any year, it is removed from the estimation sample. Note that this means that all
observations with missing data are removed from the estimation sample, and no
imputation is done in this sample. This results in only a slightly higher
number of state-months being removed for the Private and Shore modes, with a final
estimation sample size of 765 for Private and 770 for Shore. For Charter mode, the
restriction is more severe, and the restricted estimation sample contains only
600 observations.

As an additional check on the validity of the estimated number of trips which
forms the dependent variable for the study, Table \@ref(tab:PSEtable) shows the 
distribution of the Percent Standard Errors of the estimated number of trips
in each of the 3 restricted estimation samples, by showing the deciles of the 
distributions. We can see that for Shore and Private modes, the vast majority of 
observations have PSE's less than 50%, while for Charter mode there are a large
minority of such problematic observations. The noise in the charter mode observations
may thus explain why statistically significant results are not obtained in either
the main paper results or the supplementary regression results presented in this
appendix. It is important to remember however, that the regression coefficient
variances are estimated using a cluster-robust estimation method which adjusts
for the heteroskedasticity induced by this sampling variance in the dependent
variable.


```{r loadDataAppendix, warning=FALSE, echo=FALSE}
...
data_for_new_model_app <- tripsTMPS %>% 
...
data_for_new_model5_app <- tripsTMPS %>%
...
data_for_new_model3_app <- tripsTMPS %>%
  anti_join(some_no_data_sms3, by = c("State", "Month"))
```



```{r pfe_app,eval=FALSE}

rm_app=function(m,mn,y){
  
  if(m==5) {
    bd=data_for_new_model5_app
  } else if (m==3) {
    bd=data_for_new_model3_app
  }else {
    bd=data_for_new_model_app
  }
...
pm_app=rm_app(7,"Private Boats",2017:2021)
cm_app=rm_app(5,"Charter Boats",2017:2021)
sm_app=rm_app(3,"Shore",2017:2021)

#Saving model and prediction objects because the predictions take long to run.
saveRDS(pm_app,"pm_app.rds")
saveRDS(cm_app,"cm_app.rds")
saveRDS(sm_app,"sm_app.rds")

```

Table \@ref(regResultsApp) and Figure \@ref(fig:tripStringRelApp) present the
results of the Poisson fexed effects regression model estimated on the restricted
estimation sample. The results are largely identical to those reported in the main
paper.

```{r regTblApp, fig.pos="h", warning=FALSE, results='asis'}

#Reading model and prediction objects
pm_app=readRDS("pm_app.rds")
cm_app=readRDS("cm_app.rds")
sm_app=readRDS("sm_app.rds")

texreg(list(pm_app$reg,cm_app$reg,sm_app$reg),custom.model.names = c("Private","Charter","Shore"),
       booktabs = TRUE,use.packages = FALSE,threeparttable=TRUE,
       label="regResultsApp",caption.above = TRUE,digits=4,include.deviance=FALSE,include.bic=TRUE,include.loglik=FALSE,
       override.pvalues=list(pm_app$ses$p_Satt,cm_app$ses$p_Satt,sm_app$ses$p_Satt),
       caption = "Poisson Fixed Effect Regression of Trips on Covid-19 Stringency, by Mode - Restricted Estimation Sample",include.groups=FALSE,
       custom.note = "\n\\item %stars. SEs clustered by state \n\\item with small-sample adjustment.\n")

```

```{r tripStringRelApp, fig.cap="Predicted Percentage Change in Trips as a Function of the Stringency Index - Restricted Estimation Sample", fig.height=8, fig.pos="h", fig.width=4, warning=FALSE}

#Predicted trips as a function of stringency, including confidence intervals
pas=ggarrange(pm_app$pplots,cm_app$pplots,sm_app$pplots,ncol = 1, nrow = 3)
pas

```

In order to assess COVID-induced movement of fishing activity across state lines, we 
also include here a set of results that explicitly consider the number of trips
taken by residents of a particular state visiting another state. Since the MRIP
data includes information on the state of residency for anglers intercepted by
the APAIS, we can construct estimates of the number of trips taken by anglers
from a particular state to a particular state in each month. Notably, although
there are only 16 destination states covered by the MRIP program, the number of potential
origin states includes all 50 states, so for instance we can estimate the number of
trips taken by Tennessee residents to North Carolina or Florida. There are thus
potentially 800 origin-destination pairs, which when multiplied by 5 years and
12 months yields potentially 48,000 observations in the dataset for these estimations.
However, most of the origin-destination pairs are missing data for some months
due to the limited surveying done by MRIP. Thus, the actual number of observations
with positive estimated trips which can be included in regression analysis is closer
to 7,000.

In modeling the impact of COVID stringency on these cross-state trips, we can consider
both the impact of origin and destination state stringency. Theoretically, we should
anticipate that higher COVID stringency in the origin state will mean that people
have less opportunities for recreation locally, and will be more inclined to visit
other states for fishing trips, and thus trips will be increasing in origin state
stringency. Conversely, we should anticipate that higher stringency in the destination
state will lead to fewer trips to that state. Specifically, we can model the number
of trips from origin state $i$ to destination state $j$ as:

$$\mu_{ijmy} = W_{i}*\exp(\beta_{origin} S_{imy} + \beta_{destination} S_{jmy} + \alpha_{ij}).$$

Where $\mu_{ijmy}$ is the expected number of trips for origin-destination pair
$i,j$, $W_i$ is the population of origin state $i$, $S$ is the the COVID stringency
prevailing in the origin and destination states respectively, and $\alpha_{ij}$
is a fixed effect for the origin-destination pair. 

```{=latex}

\begin{table}
\label{od_reg}
\caption{Origin-Destination Regressions}
\centering
\begin{tabular}{lccc}
   \tabularnewline \midrule \midrule
   Dependent Variable: & \multicolumn{3}{c}{Trips}\\
   Model:                          & Private                     & Charter             & Shore\\  
   \midrule
   \emph{Variables}\\
   Stringency (Origin)              & 0.0029                  & 0.0530$^{***}$  & -0.0071\\   
                                   & (0.0058)                & (0.0106)        & (0.0071)\\   
   Stringency (Destination)         & -0.0022                 & -0.0401$^{***}$ & 0.0103\\   
                                   & (0.0060)                & (0.0120)        & (0.0076)\\   
   Stringency (Origin$)^2$       & $-2.88\times 10^{-5}$   & -0.0007$^{***}$ & $8.69\times 10^{-5}$\\    
                                   & ($8.47\times 10^{-5}$)  & (0.0001)        & ($8.61\times 10^{-5}$)\\    
   Stringency (Destination$)^2$  & $2.25\times 10^{-5}$    & 0.0005$^{***}$  & -0.0001\\   
                                   & ($8.14\times 10^{-5}$)  & (0.0001)        & ($9.15\times 10^{-5}$)\\    
   \midrule
   \emph{Fixed-effects}\\
   O-D pair                      & Yes                     & Yes             & Yes\\  
   \midrule
   \emph{Fit statistics}\\
   Observations                    & 6,710                   & 7,234           & 7,177\\  
   Pseudo R$^2$                    & 0.93631                 & 0.67904         & 0.89668\\  
   \midrule \midrule
   \multicolumn{4}{l}{\emph{Clustered (O-D pair) standard-errors in parentheses}}\\
...
We can estimate poisson fixed effect regression models of this form separately
for each of the 3 modes of fishing, as in the main paper. The results are presented
in Table A3. The findings are in line with the hypothesis discussed
above, with the most precise results obtaining for the charter mode, which is sensible
as charter trips are most likely to be taken by those visiting from out of state.
Quantitatively speaking, the coefficients of 0.0055 and -0.0041 for the impact of
origin and destination state stringency respectively, can be interpreted by considering
a given change in COVID stringency. For instance, New York substantially decreased its
stringency in Spring of 2021 (as shown in Table \@ref(fig:pString)), from about 65 
points to about 40 points, about a 25 
point change. The results suggest charter trips taken by New York residents to other
states would have decreased by around 14% as a result of this decrease in stringency.
Florida experienced a similar drop in stringency at that time, and the results would
suggest that charter trips to Florida from other states would have increased by
10% as a result of that change.
