...
the winter months in colder states. There are a total of 35 state-months for
...
have no data collection for the charter mode, leading to 735 observations. The only states for which data is 
collected in all months for all modes are Florida, Alabama, and North Carolina.
...
locations by field workers, and asks for information about the fishing trip they are on when surveyed, as well as sampling some of the fish they catch in order to get valid length information for the caught fish, as well as species composition data. During 2020 the FES was able to continue unhindered, while the APAIS was suspended for at least some
...
We are able to construct the total trip estimates for all state-months in 2020 in the same fashion as in 2019 and earlier years, since the FES was able to continue unhindered. However, other outcome variables which might have been investigated,
...
We model the relationship between the aggregate monthly number of salwater fishing trips trips taken in each state and the monthly COVID policy stringency using a Poisson regression model with the following conditional mean:
...
a state-specific linear time trend.

We estimate this model as a generalized linear model, using the `fixest` package 
in `R` @berge2018Efficient. Regarding variance estimation, we make several deviations from the standard
glm estimation and inference in order to account for the nature of our data. First, we calculate
...
trips to be postponed to the following month. Second, our clustered variance estimates
...
method proposed by @bell2002bias, as implemented in the `clubSandwich` package in `R`
...
our estimated Poisson regression model. A common trend is seen between
...
at the 1% level for private mode trips, and at the 5% level for shore mode, but not for charter. 
...
relationship we find to be the outcome of countervailing forces: (1) stringency reduces available substitutes of indoor
