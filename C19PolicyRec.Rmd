...
tripsT=svyby(~one,~YEAR+month+MODE_FX+ST,FUN=svytotal,design=tripss,drop.empty.groups=FALSE,na.rm=TRUE)

#Estimated trips by year, month, mode, and state
tripsM=svyby(~HRSF+PARTY+MILES,~YEAR+month+MODE_FX+ST,FUN=svymean,design=tripss,drop.empty.groups=FALSE,na.rm=TRUE)
...
tripsTM$Miles_pse=with(tripsTM,ifelse(MILES>0,se.MILES/MILES,NA))
...
names(tripsTMPS)=c("Year","Month","Mode","Trips","Trips_se","Hrsf","Party","Miles",
                   "Hrsf_se","Party_se","Miles_se",
                   "Trips_pse","Hrsf_pse","Party_pse","Miles_pse",
...
plot(y=sdm$pred,x=sdm$Stringency,type="l",ylim=c(-1,1),
     main="Shore",
     xlab="Stringincy Index (Monthly sum of daily records: higher is more stringent)",
     ylab="Mean Distance Traveled (Scaled to June in FL)")
