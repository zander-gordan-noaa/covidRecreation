...
abstract: |
  Governments responded to the Covid-19 pandemic with different policies to curtail the spread of the virus. We show how sportfishing levels are related to the stringency of Covid-19 policies. Specifically, we measure the total
  number of resident sportfishing trips taken each month in each of 16 U.S. states covered by the 
  Marine Recreational Information Program, and relate this to a state-level index
  of COVID policy stringency developed by the Oxford COVID Government Response Tracker
  program. We model the number of recreational fishing trips taken in each state-month
  as non-linear function of the stringency index, using a fixed effect Poisson
  regression model with state-specific seasonality and time trends. We
  estimate separate models for different fishing modes, and for fishing trips
  taken on private boats find that at moderate levels of stringency the number
  of trips may have increased by approximately 20%, while at high levels of stringency
  ...
  boats, though with larger point estimates of the impacts but greater uncertainty
...
length of time nationwide in March and April 2020, with data collection resuming unevenly throughout the country in May and June (wave 3) according to local conditions^[see Figure 6 of the [MRIP survey design manual](https://media.fisheries.noaa.gov/2021-09/MRIP-Survey-Design-and-Statistical-Methods-2021-09-15.pdf) for more information on which states resumed data collection at which times.].
...
Given the above specification for the conditional mean function, there are multiple
regression models which may be chosen, including a basic generalized linear model,
as well as poisson and negative binomial models that specifically model the count
nature of the dependent variable and guarantee that the fitted model produces 
non-negative predictions. We choose a Poisson regression model, following the advice
of @wooldridge2010econometric who notes that estimating a Poisson model is the most
efficient method which is consistent when only the conditional mean needs to be correctly specified,
while the negative binomial models are inconsistent unless both the conditional
mean and variance assumptions for that model are correct. The conventional drawback to Poisson
estimation is the restrictive assumption that the conditional mean and variance
are equal, which is addressed partly by the used of fixed effects which absorb
much of the variation, bringing the conditional variance closer to the conditional 
mean, and furthermore by the use of cluster-robust variance estimation, detailed below.

We estimate this Poisson regression model using the `fixest` package 
in `R` @berge2018Efficient. We calculate
...
our estimated Poisson regression models, with the state-specific time trends and 
seasonality patterns suppressed to focus on the coefficients of interest. A common trend is seen between
...
in all 3 independent models based on separate datasets.
...
In this paper, we measure the net effect of COVID policy stringency on recreational marine fishing effort, among 16 states in the eastern U.S. To do so, we rely on data from the MRIP program to measure the total number
