...
Saltwater angling is a major outdoor activity in the U.S., with expenditures on fishing trips on totaling more than $10 billion annually
...
The MRIP program oversees survey research to monitor the level of marine angling activity in a consistent fashion throughout US fisheries (we discuss the structure of these surveys in greater detail below). The specific data product we use from MRIP is the Fishing Effort Survey (FES), a mail survey which asks anglers to recall how many trips they have been on recently, in order to get information on the total level of fishing activity. The primary unit of measurement for our aggregated summary of the FES dataset is the angler-trip (hereafter abbreviated simply to trip), representing the total number of distinct trips multiplied by the average party size of those trips.

We use the FES data for the 16 states along the Gulf and Atlantic coasts in which it is available.^[Specifically, the states are: AL, CT, DE, FL, GA, ME, MD, MA, MS, NH, NJ, NY, NC, RI, SC and VA] The MRIP program publishes estimates of fishing activity for 2-month periods referred to as waves, but rather than use these estimates we use the publicly-available microdata files^[The MRIP data files are maintained by NOAA fisheries at the following URL:
...
resident fishing trips is necessary for our goal of estimating the relationship
between trips taken and COVID stringency, because if we considered all trips, then
for out-of-state trips the relevant independent variable would be either the stringency
level prevailing at the trip-taker's origin, or else the difference between stringency
at the origin versus the destination. While estimating such models would be possible in
theory, in practice the level of detail in the MRIP data is not sufficient to calculate
separate estimates of the number of trips taken by trip-takers from the various
different states at the monthly level.^[With 16 states, we would need to calculate 256 trip estimates to
handle each origin-destination pair in the sample, and that is without considering
trips taken by anglers from states not covered by MRIP, such as anglers from Tennessee
taking a trip in Florida.]
...
modes includes only 785 observations, and 45 state-months
...
In order to place the MRIP data on a comparable basis across states, we include information
...
We model the relationship between the aggregate monthly number of salwater fishing trips trips taken in each state and the monthly COVID policy stringency using a fixed effect Poisson regression model with the following conditional mean:

$$\mu_{imy} = W_{iy}*\exp(\beta_1 S_{imy} + \beta_2 S_{imy}^2 + \alpha_{im} + \gamma_i y)$$
where for state $i$ during month $m$ and year $y$, $\mu_{imy}$ is the expected aggregate trips, $W_{iy}$ is the state population, and $S_{imy}$ is the daily average COVID policy stringency. The coefficients $\beta_1$ and $\beta_2$ capture the aggregate relationship between stringency and trips taken, while the term $\alpha_{im}$ represents state-month fixed effects
...
this relationship at higher levels of stringency. The first order relationshipss are significant
...
various possible levels of COVID strignency. The solid line
...
That is, since the period of moderate stringency were immediately preceded by
...
seasonality trends and time trends in the level of fishing trips.
...
The dataset used in the main panel does not represent a full, balanced panel of
...
for its Charter mode data, which has completed intercept survey data in 2018, but
